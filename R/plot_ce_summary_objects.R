...
calculate_icers <- function(
  ...
  if(sum(is.na(df_icer$dominance)) > 0){
    ...
    ## Identify first reference intervention if ICERs were estimated
    if(sum(!is.na(subset_data$icer)) > 0) {
...
#'
#' @param .df_cea_results_ Object of class "cea_tbl" returned by
...
generate_gt_table <- function(
  ...
  ## Sanity checks:
  assertthat::assert_that(
    "cea_tbl" %in% class(.df_cea_results_),
    msg = paste(
...
#'
#' @return A object of class gt_tbl or a dataframe of class "cea_tbl"
...
plot_cea_results_table <- function(
  ...
  if(isTRUE(.format_values_)) {
    ...
    for (costs_col in currency_labeled_columns) {
      ...
      if(
        all(
          length(df_cea_results[!is.na(df_cea_results[[costs_col]]), costs_col]) == 0,
          .output_format_ == "long"
        )
      ) {
    ...
    for (effs_col in effects_columns) {
      ...
      if(
        all(
          length(df_cea_results[!is.na(df_cea_results[[effs_col]]), effs_col]) == 0,
          .output_format_ == "long")
      ) {
    ...
    if(length(pce_columns) > 0) {
      for (prb_col in pce_columns) {
        ...
        if(
          all(
            length(df_cea_results[!is.na(df_cea_results[[prb_col]]), prb_col]) == 0,
            .output_format_ == "long")
        ) {
  ...
  # Assign "cea_tbl" to the df_cea_results class:----
  class(df_cea_results) <- c(class(df_cea_results), "cea_tbl")
