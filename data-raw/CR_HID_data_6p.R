# HID Model calibration data - Six parameters ("mu_e", "mu_l", "mu_t", "p", "r_l", "rho")
...
v_params_true_values <- c('mu_e' = 0.04, 'mu_l' = 0.15, 'mu_t' = 0.016,
                          'p' = 0.12, 'r_l' = 0.41, 'rho' = 0.53)
### Prior distributions:----
#### Distribution names:----
v_params_dists <- c('mu_e' = "unif", 'mu_l' = "unif", 'mu_t' = "unif",
                    'p' = "unif", 'r_l' = "unif", 'rho' = "unif")
v_params_dists2 <- c("lnorm", "lnorm", "lnorm", "lnorm", "lnorm", "lnorm")
...
CR_HID_data_6p <- list('l_params' = l_params,
                    'l_targets' = l_targets)
## Save calibration data internally:----
usethis::use_data(CR_HID_data_6p, overwrite = TRUE)
