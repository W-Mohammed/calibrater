# HID Model calibration data - Four parameters ("mu_e", "mu_l", "mu_t", "p")
...
## Parameters:----
v_params_names <- c("mu_e", "mu_l", "mu_t", "p")
v_params_names <- c(
...
v_params_dists <- c('mu_e' = "unif", 'mu_l' = "unif", 'mu_t' = "unif",
                    'p' = "unif")
v_params_dists2 <- c("lnorm", "lnorm", "lnorm", "lnorm")
...
CR_HID_data_4p <- list('l_params' = l_params,
                    'l_targets' = l_targets)
## Save calibration data internally:----
usethis::use_data(CR_HID_data_4p, overwrite = TRUE)
