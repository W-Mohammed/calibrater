# CRS Model calibration data - Two target (Survival, PropSick)
# Targets simulated using 10% of true parameters'
# values as the sd in a PSA analysis

## Targets:----
v_targets_names <- c("Surv", "PropSick")
v_targets_labels <- c('Surv' = "Survival", 'PropSick' = "Prevalence")
v_targets_axis <- list('Surv' = list("x" = 'value', "y" = 'Year'),
                       'PropSick' = list("x" = 'value', "y" = 'Year'))
v_targets_weights <- c('Surv' = 1, 'PropSick' = 1)
### Likelihood distributions:----
v_targets_dists <- c('Surv' = "norm", 'PropSick' = "norm")
...
CR_CRS_data_2t <- list('l_params' = l_params,
                       'l_targets' = l_targets)
## Save calibration data internally:----
usethis::use_data(CR_CRS_data_2t, overwrite = TRUE)
