...
devtools::load_all()
calibration_parameters <- c("rho", "b")
calibration_targets <- c("Prev", "Surv", "Trt_vol")

### 1. Case study 1: three target - two parameters:----
#### Saving path:----
path = "../4. Post CR/VOI/"
...
l_VOI_ID_2P3T <- list(
  "outcomes" = VOI_ID_2P3T$PSA_results,
  "p" = VOI_ID_2P3T$PSA_samples
)
filename <- paste0(data_saving_path, "l_EVPPI_2P3T.rds")
saveRDS(l_VOI_ID_2P3T, file = filename)
...
params_ <- purrr::map(
  .x = 1:length(l_EVPPI_2P3T$outcomes) %>%
    `names<-`(names(l_EVPPI_2P3T[["outcomes"]])),
  ...
  .f = function(.res_) {
    ...
    outcomes <- purrr::map(
      ...
      .f = function(.outcome_) {
        ...
        conseq_df <- l_EVPPI_2P3T[["outcomes"]][[.res_]] %>%
