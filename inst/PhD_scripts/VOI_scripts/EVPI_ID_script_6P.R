...
# Parameters: c(mu_l, mu_t, p, r_l, rho, b)
# Targets: c(Prev, Surv, Trt_vol)

## Load the calibR package:
devtools::load_all()
calibration_parameters <- c("mu_l", "mu_t", "p", "r_l", "rho", "b")
calibration_targets <- c("Prev", "Surv", "Trt_vol")

### 1. Case study 3: three target - six parameters:----
#### Saving path:----
path = "../4. Post CR/VOI/"
case_study_dir <- "Case_study_3/"
chapter_dir <- ""
image_dir <- "images/"
data_dir <- "data/"
...
l_VOI_ID_6P3T <- list(
  "outcomes" = VOI_ID_6P3T$PSA_results,
  "p" = VOI_ID_6P3T$PSA_samples
)
filename <- paste0(data_saving_path, "l_EVPPI_6P3T.rds")
saveRDS(l_VOI_ID_6P3T, file = filename)
## EVPPI:----
### Prepare data:----
filename <- paste0(data_saving_path, "l_EVPPI_6P3T.rds")
l_EVPPI_6P3T <- readRDS(file = filename)
...
params_ <- purrr::map(
  .x = 1:length(l_EVPPI_6P3T$outcomes) %>%
    `names<-`(names(l_EVPPI_6P3T[["outcomes"]])),
  ...
  .f = function(.res_) {
    ...
    outcomes <- purrr::map(
      ...
      .f = function(.outcome_) {
        ...
        conseq_df <- l_EVPPI_6P3T[["outcomes"]][[.res_]] %>%
